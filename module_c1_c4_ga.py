...
def fitness_ga(params):   # 由于是取mse最小值（但是ga算法是求最大值，为此取负数）
    C, gamma = params
    svm_model = SVR(kernel='rbf', gamma=gamma, C=C)
...
def fitness_ga2(params):   # 由于是取mse最小值（但是ga算法是求最大值，为此取负数）
    C, gamma = params
    svm_model = SVR(kernel='rbf', gamma=gamma, C=C)
...
# ①原始的SVR模型
# module_orgin = SVR(kernel='rbf')

# ②GA优化参数的SVR模型
from Optimizer import GA_SVR

# n_chrom = 16 # 染色体个数
# n_iteration_ga = 20 # 轮次
# options_ga = [0.5, 0.01, 0.1]  # 交叉率，变异率，α等参数

# best_c_ga, best_gamma_ga = GA_SVR.GeneticAlgorithm(n_chrom, n_iteration_ga, bounds, dimension, fitness_ga, options_ga).solve()
# best_c_ga, best_gamma_ga = GA2.GA(n_chrom, n_iteration_ga, bounds, dimension, fitness_ga2, options_ga)

# module_ga = SVR(kernel='rbf', gamma=best_gamma_ga, C=best_c_ga)
...
# 开始训练svr
# module_orgin.fit(X_train, Y_train)
# module_ga.fit(X_train, Y_train)
module_pso.fit(X_train, Y_train)
module_ho.fit(X_train, Y_train)

# # 5.输出结果最佳超参数和评估指标
# Y_pred_train = module_orgin.predict(X_train)
# Y_pred_test = module_orgin.predict(X_test)
# print("Orign训练集MSE:", mean_squared_error(Y_train, Y_pred_train))
# # R2值表示模型解释的变异性的比例，范围从负无穷到1
# # 接近1的R2值表示模型很好地解释了数据的变异性（即异常值）
# print("Orign训练集R2:", r2_score(Y_train, Y_pred_train))
# print("Orign测试集MSE:", mean_squared_error(Y_test, Y_pred_test))
# print("Orign测试集R2:", r2_score(Y_test, Y_pred_test))

# print('--------------------------------------------------------')
#
# print("GA的最优超参数：", best_c_ga, best_gamma_ga)
# Y_pred_train = module_ga.predict(X_train)
# Y_pred_test = module_ga.predict(X_test)
# print("GA训练集MSE:", mean_squared_error(Y_train, Y_pred_train))
# # R2值表示模型解释的变异性的比例，范围从负无穷到1
# # 接近1的R2值表示模型很好地解释了数据的变异性（即异常值）
# print("GA训练集R2:", r2_score(Y_train, Y_pred_train))
# print("GA测试集MSE:", mean_squared_error(Y_test, Y_pred_test))
# print("GA测试集R2:", r2_score(Y_test, Y_pred_test))
