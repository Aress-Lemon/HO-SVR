...
# 2.划分训练集(c4,c6)，测试集(c1)
# ignore_index=True: 忽略原来的索引，并重新生成一个新的连续的整数索引
df_c4_c6 = pd.concat([df_c4, df_c6], axis=0, ignore_index=True)
X_train = df_c4_c6.iloc[:, :-1]
Y_train = df_c4_c6.iloc[:, -1]
X_test = df_c1.iloc[:, :-1]
Y_test = df_c1.iloc[:, -1]
...
# 5. 导入模型（import）、创建模型
module = LSTM(input_size=5, hidden_size=32, num_layers=2, output_size=1).to(device)
